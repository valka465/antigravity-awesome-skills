...
description: Use when a task needs multiple agents working in parallel, involves large codebases, risks hitting quota limits, or requires coordination across multiple files, services, or domains. Activates intelligent task splitting, quota-aware routing, context isolation per agent, and error recovery. Do not use for single-file edits or simple one-step tasks.
version: 1.0.0
author: community
